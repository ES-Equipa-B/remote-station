...
int sendSms(string msg) {
  ...
  // 7-01-2023
  // syncronize ESP clock everytime a SMS is sent.
  // Not worked, multiple 00h00 when required network time
...
int sendCntSMS(int &sms_sent_cnt) {
  ...
  String msg = "SMS sent in the last day: " + String(sms_sent_cnt);

  while (!modem.sendSMS(SMS_TARGET_TEST, msg))
  {
    //SerialMon.println("SMS failed to send");
    delay(500);
  }
  //SerialMon.println("CNT SMS successfully sent.");
  return 0;
}
...
bool compareTime(int t_now, int t_lastSync) {


  int now = t_now;
  int lastSync = t_lastSync;

  //SerialMon.print("Now:");
  //SerialMon.print(now);
  //SerialMon.print(" Last:");
  //SerialMon.println(lastSync);

  int now_H = now / 100;
  int last_H = lastSync / 100;

  if ( now < lastSync)
  { // Deu volta ao relogio 23h -> 00h
    lastSync = lastSync - 2400 + 40;
  } else  if (now_H != last_H) {
    // Avançou de hora 14h -> 15h, p.e
    now = now - 40;
  }

  if (now - lastSync >= 30 ) {
    //Serial.println("Passaram 30min!");
    return true;
  }

  // not passed 30 min
  //Serial.println("Não passaram 30min!");
  return false;
}
...
int sendAllSmsInBuffer_sms_test(queue<string> &buffer, int &lastSync, int &sms_sent_cnt, int &sync_min) {
  ...
  getLocalTime(&now, 0);
  sync_min = now.tm_min;
  //SerialMon.println("(Sync)Seconds now:" + String(now.tm_sec));
  //SerialMon.println("(Sync)now_min:" + String(now.tm_min));
...
std::string getLocalHourMinutes() {
  string hhmm;
  struct tm now;
  getLocalTime(&now, 0);
  ostringstream oss;
...
int syncRtcWithNetworkTime(bool first_exec) {
  ...
  /*
    ...
    wakeUpGsm();
    SerialMon.println("Forçou o wakeup qd ja tava acordado!");
    }*/
  //delay(5000);
  ...
  if (first_exec) {
    ...
    */
    int yr = stoi(gsm_datetime.substr(0, 2));
    int month = stoi(gsm_datetime.substr(3, 2));
    int mday = stoi(gsm_datetime.substr(6, 2));
    int hr = stoi(gsm_datetime.substr(9, 2));
    int minute = stoi(gsm_datetime.substr(12, 2));
    int sec = stoi(gsm_datetime.substr(15, 2));
    int isDst = stoi(gsm_datetime.substr(18, 2));
    ...
    struct tm beforeSync;
    int i = 0;
    while (i < 2) {

      getLocalTime(&beforeSync, 0);
      string gsm_datetime = modem.getGSMDateTime(DATE_FULL).c_str();
      ...
      time_t t = mktime(&tm);
      time_t before_t = mktime(&beforeSync);
      if (difftime(t, before_t) < 5 * 60) // se passaram menos de 5min deixa sincronizar
      {
        struct timeval now = { .tv_sec = t };
        settimeofday(&now, NULL);
        break;

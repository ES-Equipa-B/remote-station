...
RTC_DATA_ATTR bool slept = false;
...
void Task1code(void *pvParameters) {
  ...
  for (;;) {
    ...
    lcd.setCursor(0, 0);
    // print message
    lcd.print(String("ON_t") +String(lastSync) + String(" T1:") + String(state_t1));
    lcd.setCursor(0, 1);
    // print message
    lcd.print(String("now") + String(getLocalHourMinutes().c_str()) + String(" T2:") + String(state_t2));
    //delay(5000);
    ...
    switch (state_t1) {
      ...
      case (S_T1_Setup):
        ...
        if (sms_sent == -1) {
          ...
          //SerialMon.println(String("(init)boot_time: ") + String(boot_time));
          //SerialMon.println(String("(init)lastSync: ") + String (lastSync));
          //sms_sent = 0;
        }
        getLocalTime(&now, 0);
      ...
      case (S_T1_StoreSample):
        ...
        // Verify it was passed 1 day
        //SerialMon.println(String(getLocalHourMinutes().c_str()));
        // 7-01-2023
        //SerialMon.println(String(boot_time.c_str()));
        //SerialMon.println(sms_sent);
        // 8-01-2023 - Nao se pode fazer isto senão pode encravar a main thread e nao vai dormir depois
        /*
          if (String(getLocalHourMinutes().c_str()) == boot_time.c_str()) {
          wakeUpGsm();
          sendCntSMS(sms_sent);
          // reset timer
          sms_sent = 0;
      ...
      case (S_T1_PrepareSms):
        ...
        std::string dbg_sms;
        dbg_sms <<  "ESP_t:" << getLocalHourMinutes() << " #samples" << measure_cnt << " #sms_sent" << sms_sent;
        sms_buffer.push();
      ...
      case (S_T1_Sleep):
        ...
        if (!slept) {
          ...
          //SerialMon.println(String("(Sleep)now: ") + String (t_now));
          //SerialMon.println(String("(Sleep)boot_time: ") + String(boot_time));
          //SerialMon.println(String("(Sleep)sync_min: ") + String(sync_min));
          if (t_now == boot_time) {
          ...
          getLocalTime(&now, 0);
          //SerialMon.println("(Sleep)Seconds now:" + String(now.tm_sec));
          //SerialMon.println("(Sleep)now_min:" + String(now.tm_min));
          ...
          //SerialMon.println("(Sleep)ESP will sleep for " + String(sleep_t));
          slept = 1;
          ...
          //esp_sleep_enable_timer_wakeup(5000 * 2000); // Sleep for 2 second to debug
          ...
          // 7-01-2023
          // 4-01-2023
          // For sync the ESP clock every 1h
          /*
            if ( (sms_sent % SMS_PER_HOUR) == 0) {
            state_t1 = S_T1_Setup;
            }*/

          //8-01-2023
          // For sync the ESP clock every 30min
          /* TESTED AND WORKED, only sync 1 time!
            if ( (((sms_sent % SMS_PER_HOUR) == 0) || (sms_sent % SMS_PER_HOUR) == (SMS_PER_HOUR / 2)) && !synchronized) {
            state_t1 = S_T1_Setup;
            synchronized = true;
            }
            if (synchronized && (sms_sent % SMS_PER_HOUR) == 1) {
            synchronized = false;
            }
          */

...
void setup()
{
  ...
  D_SERIAL.println("Both tasks created!");
  bootCount++;
}
...
void Task1code(void *pvParameters) {
  ...
  D_SERIAL.println("Task1 running on core" + String(xPortGetCoreID()));
    ...
    switch (state_t1) {
      ...
      case (S_T1_Setup):
        ...
        if (sms_sent == -1) {
          ...
          D_SERIAL.println(String("(init)boot_time: ") + String(boot_time));
          D_SERIAL.println(String("(init)lastSync: ") + String (lastSync));
          sms_sent = -1;
        }
        getLocalTime(&now, 0);
      ...
      case (S_T1_StoreSample):
        ...
        D_SERIAL.println("S_T1_StoreSample");
        measures_cnt++;
        D_SERIAL.println("Measure counter: "  + String(measures_cnt));
      ...
      case (S_T1_PrepareSms):
        ...
        D_SERIAL.println("S_T1_PrepareSms");
        sms_buffer.push(sms); // Push current SMS to queue
        strcpy(sms, ""); // Delete current SMS
        D_SERIAL.println("SMS queue size: "  + String(sms_buffer.size()));
        ...
        /*
          ...
          sms_buffer.push(dbg);
          D_SERIAL.println(String("(PrepSMS)debug sms ") + String(dbg.c_str()));
        */
        // -------------------------- !>
      ...
      case (S_T1_TriggerSmsTask):
        ...
        if (sms_task_trigger == 0) {
          D_SERIAL.println("S_T1_TriggerSmsTask");
          ...
          if (sms_task_ended || sms_task_duration > sms_task_timeout) {
            ...
            D_SERIAL.println("SMS Task ended or timeout reached.");
            sms_task_trigger = 0;
            sms_task_ended = 0;
            state_t1 = S_T1_Sleep;
          }
      ...
      case (S_T1_Sleep):
        ...
        D_SERIAL.println("S_T1_Sleep");

        if (!slept) {
          // Sleep
          std::string s_now = getLocalHourMinutes();
          int t_now = stoi(s_now.c_str());
          D_SERIAL.println(String("(Sleep)now: ") + String (t_now));
          D_SERIAL.println(String("(Sleep)boot_time: ") + String(boot_time));
          D_SERIAL.println(String("(Sleep)sync_min: ") + String(sync_min));

          if (t_now == boot_time) {
            String msg = String("SMS sent in the last day: ") + String(sms_sent);
            sms_buffer.push(msg.c_str());
            D_SERIAL.println(msg);
            sms_sent = -1;
          }

          //Copy queue to memory before sleep
          D_SERIAL.println("Copying queue to memory.");
          sms_buffer_size = sms_buffer.size();
          while (!sms_buffer.empty()) {

            D_SERIAL.println("Copying (" + String(sms_buffer.front().c_str()) + ") to position " + String(10 - sms_buffer.size()));

            strcpy(sms_buffer_rtc[10 - sms_buffer.size()], sms_buffer.front().c_str());
            sms_buffer.pop();
          }
          for (int i = 0; i < (10 - sms_buffer_size); i++) {
            D_SERIAL.println("Erasing position " + String(i));
            D_SERIAL.println("Position string: " + String(sms_buffer_rtc[i]));
            strcpy(sms_buffer_rtc[i], "");
          }
          getLocalTime(&now, 0);

          //D_SERIAL.println("(Sleep)Seconds now:" + String(now.tm_sec));
          //D_SERIAL.println("(Sleep)now_min:" + String(now.tm_min));
          ...
          //D_SERIAL.println("Seconds now:" + String(now.tm_sec));
          //sleep_t = 60000 - (now.tm_sec * 1000);
          //sleep_t = 60000 - (millis() - store_sample_start_t);
          D_SERIAL.println("ESP will sleep for " + String(sleep_t));
          slept = 1;
          D_SERIAL.println("ESP is sleeping.");
          ...
          D_SERIAL.println("Copying memory to queue.");
          sms_buffer.empty();

          for (int i = 0; i < 10; i++) {
            if (sms_buffer_rtc[i][0] != '\0') {
              strncpy(sms_to_push, sms_buffer_rtc[i], 160);
              D_SERIAL.println("Pushing the index " + String(i) + "(" + String(sms_to_push) + ") to queue because it is not null.");
          ...
          if (measures_cnt == (int)(60 / SMS_PER_HOUR)) {
            ...
            D_SERIAL.println("ESP woke up, with 10 samples obtained.");
            measures_cnt = 0;
          }
          else {
            D_SERIAL.println("ESP woke up, with less than 10 samples obtained.");
          }
...
void Task2code (void * pvParameters) {
  ...
  D_SERIAL.println("Task2 running on core" + String(xPortGetCoreID()));
  ...
  for (;;) {
    ...
    switch (state_t2) {
      ...
      case (S_T2_Wait):
        ...
        D_SERIAL.println("S_T2_Wait");

        if (sms_task_trigger == 1) {
          D_SERIAL.println("Sms task triggered.");
      ...
      case (S_T2_SendSms):
        // Sms task triggered
        D_SERIAL.println("S_T2_SendSms");
        printQueue(sms_buffer);

        if (sms_buffer.size() == 0) {
          D_SERIAL.println("There are no SMS in buffer.");
          sms_task_ended = 1;
          state_t2 = S_T2_Wait;

        }
        else {
          D_SERIAL.println("There are SMSs in buffer.");
          printQueue(sms_buffer);
          ...
          if (sendAllSmsInBuffer_sms_test(sms_buffer, lastSync, sms_sent, sync_min) == 0) {
            ...
            D_SERIAL.println("All SMSs in buffer were sent.");
            printQueue(sms_buffer);
            sms_task_ended = 1;
            state_t2 = S_T2_Wait;
